...
L{1} = L0;
W{1} = W0;
[f{1},g1,g2,H1,H2,H12] = fun(L{1},W{1});
grad_norm{1} = sqrt((g1.^2)+(g2.^2));

for iters = 1:maxit
    work_ind = find(grad_norm{iters}>tol);
    work_size = length(work_ind);
    if isempty(work_ind)
        break;
    end
    % diag loading for problems with negative lambda
    lamda_min = 0.5*((L{iters}(work_ind)).^2+(W{iters}(work_ind)).^2+2*c)...
              -0.5*sqrt(((L{iters}(work_ind)).^2+(W{iters}(work_ind)).^2).^2+12*((W{iters}(work_ind)).^2).*((L{iters}(work_ind)).^2));    
    ...
    switch modif_mode
        ...
        case 'reverse'
            lamda_max = 0.5*((L{iters}(work_ind)).^2+(W{iters}(work_ind)).^2+2*c)...
              +0.5*sqrt(((L{iters}(work_ind)).^2+(W{iters}(work_ind)).^2).^2+12*((W{iters}(work_ind)).^2).*((L{iters}(work_ind)).^2));
    ...
    t = BTLS_multisolver(fun, L{iters}(work_ind), W{iters}(work_ind), X_l(work_ind), X_w(work_ind), c, -dir1, -dir2, 0.25, 0.5);

    % update 
    L{iters+1} = L{iters};
    W{iters+1} = W{iters};
    L{iters+1}(work_ind) = L{iters}(work_ind) - t.*dir1;
    W{iters+1}(work_ind) = W{iters}(work_ind) - t.*dir2;

    % calculate next values
    [f{iters+1},g1,g2,H1,H2,H12] = fun(L{iters+1},W{iters+1});
    grad_norm{iters+1} = sqrt((g1.^2)+(g2.^2));

    % print 
    fprintf('i:%2d|mt:%2.4f|Mt:%2.4f|mg:%1.2e|Mg:%1.2e|w_size:%2d\n',iters,min(t),max(t),min(min(grad_norm{iters+1})),max(max(grad_norm{iters+1})),work_size);
